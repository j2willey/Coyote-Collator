# Camporee Configuration Schema (v2.9)
...
### Attributes
* **`schemaVersion`**: (String) Version identifier (currently "2.9").
* **`meta`**: (Object) High-level event details.
    * `camporeeId`: (UUID) Unique identifier for the event (used for server storage).
